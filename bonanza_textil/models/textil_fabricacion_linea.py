...
class TextilFabricacionLinea(models.Model):
    ...
    @api.depends('product_id', 'lot_id', 'location_id', 'fabricacion_id.partner_id')
    def _compute_cantidad_disponible(self):
        Quant = self.env['stock.quant']
        for line in self:
            if not (line.product_id and line.lot_id and line.location_id and line.fabricacion_id.partner_id):
